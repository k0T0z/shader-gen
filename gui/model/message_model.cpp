...
void MessageModel::build_sub_models() {
  ...
  for (int i{0}; i < total_fields; i++) {
    ...
    SILENT_CONTINUE_IF_TRUE(m_sub_models_by_field_number.find(field_desc->number()) !=
                            m_sub_models_by_field_number.end());
...
QVariant MessageModel::data() const { FAIL_AND_RETURN_NON_VOID(QVariant(), "Cannot get data from MessageModel."); }
...
QModelIndex MessageModel::parent([[maybe_unused]] const QModelIndex& child) const {
  ...
  FAIL_AND_RETURN_NON_VOID(QModelIndex(),
                           "Parent model is not a repeated message model, message model, or oneof model.");
}
...
bool MessageModel::setData(const QModelIndex& index, const QVariant& value, int role) {
  ...
  if (shadergen_utils::is_inside_real_oneof(field_desc)) {
    ...
    return const_cast<ProtoModel*>(get_sub_model(field_desc->number()))
        ->setData(this->index(0, map_to_oneof_index(index.column()), index), value, role);
  }

  return const_cast<ProtoModel*>(get_sub_model(field_desc->number()))->setData(this->index(0, 0, index), value, role);
}

QVariant MessageModel::headerData(int section, [[maybe_unused]] Qt::Orientation orientation,
