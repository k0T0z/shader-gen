...
class AIAgentParametersEditor : public QWidget {
 ...
 public:
  ...
  float get_mutation_probability() const { return parameter_line_edits[1]->text().toFloat(); }
  float get_crossover_probability() const { return parameter_line_edits[2]->text().toFloat(); }
  float get_elitism_ratio() const { return parameter_line_edits[3]->text().toFloat(); }
  int get_maximum_iterations() const { return parameter_line_edits[4]->text().toInt(); }
