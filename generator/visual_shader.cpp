...
bool VisualShader::add_node(const std::shared_ptr<VisualShaderNode>& node, const Vector2& coordinate, const int& id) {
  if (!node) {
...
Vector2 VisualShader::get_node_coordinate(const int& id) const {
  const VisualShader::Graph* g{&graph};
...
bool VisualShader::set_node_coordinate(const int& id, const Vector2& coordinate) {
  VisualShader::Graph* g{&graph};
...
bool VisualShader::generate_shader_for_each_node(std::string& global_code, std::string& global_code_per_node,
  ...
  for (int i{0}; i < input_port_count; i++) {
    ...
    if (input_connections.find(key) != input_connections.end()) {
      ...
      Variant defval{n->get_input_port_default_value(i)};

      switch (defval.index()) {
        case VARIANT_FLOAT_INDEX: {  // float
        ...
        case VARIANT_VECTOR_2D_INDEX: {  // Vector2
          Vector2 val{std::get<Vector2>(defval)};
          input_vars.at(i) = "var_to_n" + std::to_string(node_id) + "_p" + std::to_string(i);
          std::ostringstream oss;
        ...
        case VARIANT_VECTOR_3D_INDEX: {  // Vector3
          Vector3 val{std::get<Vector3>(defval)};
          input_vars.at(i) = "var_to_n" + std::to_string(node_id) + "_p" + std::to_string(i);
          std::ostringstream oss;
        ...
        case VARIANT_VECTOR_4D_INDEX: {  // Vector4
          Vector4 val{std::get<Vector4>(defval)};
          input_vars.at(i) = "var_to_n" + std::to_string(node_id) + "_p" + std::to_string(i);
          std::ostringstream oss;
...
void VisualShaderNode::set_input_port_default_value(const int& port, const Variant& value,
                                                    const Variant& prev_value) {
  Variant v{value};

  if (prev_value.index() != VARIANT_MONOSTATE_INDEX) {  // std::monostate
    switch (value.index()) {
      case VARIANT_FLOAT_INDEX: {  // float
        switch (prev_value.index()) {
          ...
          case VARIANT_VECTOR_2D_INDEX: {  // Vector2
            Vector2 pv{std::get<Vector2>(prev_value)};
            v = pv.x;
          } break;
          case VARIANT_VECTOR_3D_INDEX: {  // Vector3
            Vector3 pv{std::get<Vector3>(prev_value)};
            v = pv.x;
          } break;
          case VARIANT_VECTOR_4D_INDEX: {  // Vector4
            Vector4 pv{std::get<Vector4>(prev_value)};
            v = pv.x;
          } break;
          default:
            break;
      ...
      case VARIANT_VECTOR_4D_INDEX: {  // Vector4
        switch (prev_value.index()) {
          ...
          case VARIANT_VECTOR_2D_INDEX: {  // Vector2
            Vector2 pv{std::get<Vector2>(prev_value)};
            v = Vector4(pv.x, pv.y, pv.y, pv.y);
          } break;
          case VARIANT_VECTOR_3D_INDEX: {  // Vector3
            Vector3 pv{std::get<Vector3>(prev_value)};
            v = Vector4(pv.x, pv.y, pv.z, pv.z);
          } break;
          case VARIANT_VECTOR_4D_INDEX: {  // Vector4
            v = prev_value;
          } break;
...
Variant VisualShaderNode::get_input_port_default_value(const int& port) const {
