...
#ifndef ENIGMA_VISUAL_SHADER_HPP
#define ENIGMA_VISUAL_SHADER_HPP
...
bool g_are_almost_equal(const float& a, const float& b);
...
struct Vector2 {
  ...
  bool are_almost_equal(const Vector2& vector) const {
    return g_are_almost_equal(x, vector.x) && g_are_almost_equal(y, vector.y);
  }
...
struct Vector3 {
  ...
  bool are_almost_equal(const Vector3& vector) const {
    return g_are_almost_equal(x, vector.x) && g_are_almost_equal(y, vector.y) && g_are_almost_equal(z, vector.z);
  }
...
struct Vector4 {
  ...
  bool are_almost_equal(const Vector4& vector) const {
    return g_are_almost_equal(x, vector.x) && g_are_almost_equal(y, vector.y) && g_are_almost_equal(z, vector.z) &&
           g_are_almost_equal(w, vector.w);
  }
...
struct Color {
  ...
  bool are_almost_equal(const Color& color) const {
    return g_are_almost_equal(r, color.r) && g_are_almost_equal(g, color.g) && g_are_almost_equal(b, color.b) &&
           g_are_almost_equal(a, color.a);
  }

  bool operator==(const Color& c) const { return r == c.r && g == c.g && b == c.b && a == c.a; }
};

using Variant = std::variant<std::monostate, float, int, Vector2, Vector3, Vector4, bool, std::string>;

#define VARIANT_MONOSTATE_INDEX 0
#define VARIANT_FLOAT_INDEX 1
#define VARIANT_INT_INDEX 2
#define VARIANT_VECTOR_2D_INDEX 3
#define VARIANT_VECTOR_3D_INDEX 4
#define VARIANT_VECTOR_4D_INDEX 5
#define VARIANT_BOOL_INDEX 6
#define VARIANT_STRING_INDEX 7
...
class VisualShader {
 public:
   ...
   */
  bool add_node(const std::shared_ptr<VisualShaderNode>& node, const Vector2& coordinate, const int& id);
  ...
  Vector2 get_node_coordinate(const int& id) const;
  bool set_node_coordinate(const int& id, const Vector2& coordinate);
...
class VisualShaderNode {
 public:
  ...
  virtual void set_input_port_default_value(const int& port, const Variant& value,
                                            const Variant& prev_value = Variant());
  Variant get_input_port_default_value(const int& port) const;
 ...
 private:
  std::unordered_map<int, Variant> default_input_values;
...
#endif  // ENIGMA_VISUAL_SHADER_HPP
